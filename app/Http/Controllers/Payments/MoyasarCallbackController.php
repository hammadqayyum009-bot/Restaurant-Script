...
class MoyasarCallbackController extends Controller
{
    public function handle(Request $request, PaymentDriverRegistry $registry, PaymentVerificationService $verifier)
    {
        ...
        // The customer's browser landing back here after Moyasar's hosted
        // page proves nothing by itself (see class docblock) — verify()
        // above is what actually determined the real status; this redirect
        // only sends them to the screen that reports it.
        return redirect()->to(
