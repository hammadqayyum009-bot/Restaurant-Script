...
- **Checkout & orders** — delivery/pickup, a two-step checkout (order details, then payment method) supporting Cash on Delivery plus real online gateways (Moyasar, Tap), automatic retry and stuck-payment recovery, order stored in the database with a unique order number and live payment-status tracking, plus a "confirm on WhatsApp" shortcut.
- **VAT-compliant billing** — quotations, proforma and tax invoices, ZATCA Phase-1-style QR codes, credit notes, and delivery notes, gapless sequential numbering, bilingual (Arabic-primary) printing.
...
| **Orders** | Filter by status/type, open an order, move it through the status workflow (the customer is emailed automatically), delete (blocked once a payment record exists — cancel it instead) |
...
| **Ordering settings** | Delivery fee, minimum order, tax, delivery/pickup toggles, reservation hours and party size, review moderation — all enforced on the public site |
| **Payment methods** | Enable Cash on Delivery, Moyasar and/or Tap; test/live keys, per-method order limits, custom icons, retry and stuck-payment reconciliation settings |
| **Billing & documents** | Create, issue and print quotations, invoices, credit notes and delivery notes; VAT rate, seller details and numbering format under Settings → Billing |
