...
6. **Done!** Your restaurant website is now live. Sign in to manage it at
   **`https://yourdomain.com/admin/login`** with the admin account you just
   created — visiting `/admin` alone also works, redirecting you to that
   login page (or straight to the dashboard if you're already signed in).
...
- **`/admin` (or any other page) returns 404 after you update files on a live site**: this is almost always a stale route cache left over from a previous deploy, not a missing page — delete `bootstrap/cache/*.php` (everything except `.gitignore`) via File Manager, or run `php artisan route:clear` if you have shell access, then reload.
